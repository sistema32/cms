...
export function SliderListPage() {
  ...
  async function deleteSlider(id, name) {
    ...
    try {
      const response = await fetch(`/api/lexslider/sliders/${id}`, {
        method: 'DELETE'
      });
  ...
  async function duplicateSlider(id, name) {
    try {
      const response = await fetch(`/api/lexslider/sliders/${id}/duplicate`, {
        method: 'POST'
      });
    ...
    <div class="p-8 max-w-7xl mx-auto">
      <div class="flex justify-between items-center mb-8">
        ...
        <div class="flex gap-3">
          <a href="/admincp/lexslider/templates" class="inline-flex items-center gap-2 px-5 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-800 rounded-lg font-medium transition-colors no-underline">
          ...
          <a href="/admincp/lexslider/new" class="inline-flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors no-underline">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      ${sliders.length === 0 ? html`
        <div class="text-center p-16 bg-white rounded-xl border-2 border-dashed border-gray-200">
          ...
          <a href="/admincp/lexslider/new" class="inline-flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors no-underline">Crear Slider</a>
        </div>
      ` : html`
          ...
          ${sliders.map(slider => html`
            <div key=${slider.id} class="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-300 border border-gray-100">
              ...
              <div class="flex gap-2 p-4 border-t border-gray-100 bg-gray-50/50">
                <a href="/admincp/lexslider/slides/${slider.id}" class="flex-1 inline-flex justify-center items-center gap-2 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm rounded-lg font-medium transition-colors no-underline">
                ...
                <a href="/admincp/lexslider/edit/${slider.id}" class="inline-flex justify-center items-center p-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg transition-colors" title="Settings">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
