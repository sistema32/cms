...
import {
  ...
  startReconcilerLoop,
} from "./services/pluginReconciler.ts";
import { startMetricsReporter } from "./services/pluginMetrics.ts";
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    try {
      ...
      if (
        ...
        console.warn(
          "⚠️  No se pudo abrir sockets (modo sandbox). Se omitirá el servidor HTTP.",
        );
        return -1;
      }
      if (error instanceof Deno.errors.AddrInUse) {
        console.warn(
          `⚠️  Port ${testPort} is already in use, trying next port...`,
        );
        continue;
      }
      throw error;
...
try {
  ...
  if (isConnected) {
    ...
    console.warn(
      "⚠️ Email provider verification failed - emails may not be sent",
    );
  }
} catch (error) {
...
try {
  ...
  console.log(
    `✅ Backup system initialized (${stats.totalBackups} backups, ${stats.successfulBackups} successful)`,
  );
} catch (error) {
...
try {
  ...
  if (mod && typeof (mod as any).setupAutoModeration === "function") {
    ...
    console.log(
      "ℹ️  Auto-moderation plugin no encontrado; usando reglas básicas.",
    );
  }
} catch (error) {
...
try {
  ...
  console.log(
    `✅ Job queue started (${stats.waiting} waiting, ${stats.active} active)`,
  );
} catch (error) {
...
try {
  ...
  if (availablePort === -1) {
    console.warn("🪣 Sandbox sin red: servidor HTTP deshabilitado.");
  ...
  if (availablePort !== port) {
    console.warn(
      `⚠️  Configured port ${port} was in use. Starting server on port ${availablePort} instead.`,
    );
  }
...
`);
  ...
  if (error instanceof Deno.errors.PermissionDenied) {
    console.warn("Continuando sin servidor HTTP por restricciones de red.");
    await new Promise(() => {});
  } else {
    Deno.exit(1);
  }
