...
export const ContentListNexusPage = (props: ContentListNexusProps) => {
    ...
    soft: true
      ...
      <td>
        <div class="actions-cell">
          <a href="${basePath}/edit/${item.id}" class="action-btn" title="Editar">
          ...
          <button
            data-delete-url="${basePath}/${item.id}"
            class="action-btn action-btn-danger btn-delete-content"
            title="Eliminar"
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  const content = html`
    ...
    <script>
      ...
      // Status filter handler (CSP-compliant)
      const statusFilter = document.getElementById('statusFilter');
      if (statusFilter) {
        statusFilter.addEventListener('change', function() {
          const baseUrl = this.getAttribute('data-base-url');
          window.location.href = baseUrl + '?status=' + this.value;
        });
      }

      // Delete content handler (CSP-compliant)
