  ...
  return AdminLayoutNexus({
    ...
    children: html`
      ...
      <div>
        ...
        <div class="content-card u-section-hidden" id="pending-section">
              ...
              <div class="plugin-item" data-name="\${p.name}" style="border-left: 4px solid #f59e0b;">
                ...
                <div class="plugin-content">
                  ...
                  <div class="plugin-meta">
                    ...
                    <div class="flex items-center gap-2" title="Integridad del manifest">
                      ...
                      \${p.manifestChecksum
        ? '<span class="badge badge-success" title="Checksum del manifest">' + p.manifestChecksum.slice(0, 12) + '...</span>'
        : '<span class="badge badge-default">Checksum no disponible</span>'}
                      \${p.manifestSignature
        ? '<span class="badge badge-primary" title="Firma del manifest">' + p.manifestSignature.slice(0, 12) + '...</span>'
        : '<span class="badge badge-default">Sin firma</span>'}
                    </div>
                  </div>
                </div>
      ...
      async function loadPlugins() {
        ...
        try {
          ...
          container.innerHTML = plugins.map((p) => {
            ...
            const manifestHash = typeof p.manifestHash === 'string' && p.manifestHash.length > 0 ? p.manifestHash : null;
            const manifestSignature = typeof p.manifestSignature === 'string' && p.manifestSignature.length > 0
              ? p.manifestSignature
              : (typeof p.signature === 'string' && p.signature.length > 0 ? p.signature : null);
            const latencyMs = typeof p.lastHealthLatencyMs === 'number' ? Math.round(p.lastHealthLatencyMs) : null;
            const lastHealthAt = p.lastHealthCheckedAt ? new Date(p.lastHealthCheckedAt).toLocaleTimeString() : 'N/D';
            const breakerOpen = p.lastHealthError === 'breaker_open';
                ...
                <div class="plugin-content">
                  ...
                  <div class="plugin-meta">
                    ...
                    <div class="flex items-center gap-2" title="Integridad del manifest">
                      <svg class="w-4 h-4 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                      \${manifestHash
        ? '<span class="badge badge-success" title="Checksum del manifest">' + manifestHash.slice(0, 12) + '...</span>'
        : '<span class="badge badge-default">Checksum no disponible</span>'}
                      \${manifestSignature
        ? '<span class="badge badge-primary" title="Firma del manifest">' + manifestSignature.slice(0, 12) + '...</span>'
        : '<span class="badge badge-default">Sin firma</span>'}
                    </div>
                    <div class="flex items-center gap-2" title="Última comprobación de salud y breaker">
                      <svg class="w-4 h-4 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                      <span>Latencia: <strong>\${latencyMs !== null ? latencyMs + 'ms' : 'N/D'}</strong></span>
                      <span>Último check: \${lastHealthAt}</span>
                      \${breakerOpen ? '<span class="badge badge-danger">Breaker</span>' : '<span class="badge badge-success">Breaker cerrado</span>'}
                    </div>
                  </div>
                </div>
        ...
        : \`<button class="nexus-btn nexus-btn-sm \${p.status === 'active' ? 'nexus-btn-outline' : 'nexus-btn-primary'}" data-action="\${p.status === 'active' ? 'deactivate' : 'activate'}">
                  ...
                  \${p.isSystem ? '' : \`<button class="nexus-btn nexus-btn-ghost nexus-btn-sm" data-action="restart" title="Reiniciar plugin">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v6h6M20 20v-6h-6M5 19a9 9 0 0114-14"/></svg>
                  </button>\`}
    ...
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (!res.ok || !rj.success) {
          ...
          const successMsg = rj.message || (targetAction === 'activate' ? 'Plugin activado correctamente' : 'Operación exitosa');
...
async function editPermissions(name) {
  // Fetch current grants and plugin info
  const [grantsRes, pluginRes] = await Promise.all([
    fetch('/api/plugins/' + name + '/grants'),
    fetch('/api/plugins')
  ]);
  
  const grantsJson = await grantsRes.json().catch(() => ({ data: [] }));
  const pluginsJson = await pluginRes.json().catch(() => ({ data: [] }));
  
  const currentGrants = grantsJson.data || [];
  const grantedSet = new Set(currentGrants.filter(g => g.granted !== false).map(g => g.permission));
  
  // Find the plugin to get requested permissions
  const plugin = (pluginsJson.data || []).find(p => p.name === name);
  const requestedPermissions = plugin?.requestedPermissions || currentGrants.map(g => g.permission);
  
  if (!requestedPermissions.length) {
    showToast('Este plugin no tiene permisos configurados', 'error');
    return;
  }
  
  // Show modal with all permissions, pre-checking the granted ones
  const selectedPerms = await showEditPermissionModal(plugin?.displayName || name, requestedPermissions, grantedSet);
  
  if (selectedPerms === false) return; // Cancelled
  
  // Build new grants array - selected = granted, not selected = denied
  const newGrants = requestedPermissions.map(permission => ({
    permission,
    granted: selectedPerms.includes(permission)
  }));
...
// Edit permissions modal - similar to showPermissionModal but with pre-selection
function showEditPermissionModal(pluginName, permissions, grantedSet) {
  return new Promise((resolve) => {
    ...
    // Track selected permissions - start with currently granted ones
    const selectedPermissions = new Set(permissions.filter(p => grantedSet.has(p)));

    const dialog = document.createElement('dialog');
    dialog.className = 'nexus-modal';
    dialog.id = 'edit-permissions-modal';
    dialog.style.cssText = 'position: fixed; inset: 0; margin: 0; padding: 0; width: 100%; height: 100%; max-width: none; max-height: none; background-color: transparent; border: none; z-index: 999; display: grid; place-items: center;';
    dialog.innerHTML = \`
      <style>
        #edit-permissions-modal::backdrop { background: rgba(0, 0, 0, 0.4); }
        #edit-permissions-modal .perm-modal-box {
          background: white;
        ...
        #edit-permissions-modal .perm-modal-header {
          display: flex;
          align-items: center;
          justify-content: space-between;
        ...
        #edit-permissions-modal .perm-modal-title {
          font-size: 1.25rem;
          font-weight: 700;
          margin: 0;
        }
        #edit-permissions-modal .perm-modal-body {
          padding: 1.5rem;
          overflow-y: auto;
          flex: 1;
          min-height: 0;
        }
        #edit-permissions-modal .perm-modal-footer {
          padding: 1.25rem 1.5rem;
        ...
        #edit-permissions-modal .perm-close-btn {
          background: transparent;
          border: none;
          cursor: pointer;
          padding: 0.5rem;
        ...
        #edit-permissions-modal .perm-close-btn:hover { opacity: 1; background: #eef0f2; }
      </style>
      <div class="perm-modal-box">
        <div class="perm-modal-header">
          <div>
            <h3 class="perm-modal-title">⚙️ Editar Permisos</h3>
            <p style="margin: 0.25rem 0 0 0; font-size: 0.875rem; color: #666; font-weight: normal;">
              Plugin: <strong>\${pluginName}</strong>
            </p>
          </div>
          <div style="display: flex; align-items: center; gap: 0.75rem;">
            <span class="badge badge-primary">
              <span id="edit-selected-count">\${selectedPermissions.size}</span> / \${permissions.length} activos
            </span>
            <button type="button" class="perm-close-btn edit-close-btn">
        ...
        <div style="display: flex; gap: 0.75rem; padding: 1rem 1.5rem; background: var(--nexus-base-100, #fafbfc); border-bottom: 1px solid var(--nexus-base-200, #eef0f2);">
          <button type="button" id="edit-select-all-btn" class="nexus-btn nexus-btn-outline nexus-btn-sm" style="flex: 1;">
            ✓ Activar Todos
          </button>
          <button type="button" id="edit-select-none-btn" class="nexus-btn nexus-btn-outline nexus-btn-sm" style="flex: 1;">
            ✗ Desactivar Todos
          </button>
        </div>
        ...
        <div class="perm-modal-body">
          \${renderEditCategory('Database', 'db', categorized.db, '#167bff', \`
            <path d="M3 12v3c0 1.657 3.134 3 7 3s7-1.343 7-3v-3c0 1.657-3.134 3-7 3s-7-1.343-7-3z"/>
            <path d="M3 7v3c0 1.657 3.134 3 7 3s7-1.343 7-3V7c0 1.657-3.134 3-7 3S3 8.657 3 7z"/>
            <path d="M17 5c0 1.657-3.134 3-7 3S3 6.657 3 5s3.134-3 7-3 7 1.343 7 3z"/>
          \`, '#f8f9fa')}
          
          \${renderEditCategory('Filesystem', 'fs', categorized.fs, '#f59e0b', \`
            <path d="M2 6a2 2 0 012-2h5l2 2h5a2 2 0 012 2v6a2 2 0 01-2 2H4a2 2 0 01-2-2V6z"/>
          \`, '#fef3c7')}
          
          \${renderEditCategory('HTTP Routes', 'route', categorized.route, '#10b981', \`
            <path fill-rule="evenodd" d="M12.586 4.586a2 2 0 112.828 2.828l-3 3a2 2 0 01-2.828 0 1 1 0 00-1.414 1.414 4 4 0 005.656 0l3-3a4 4 0 00-5.656-5.656l-1.5 1.5a1 1 0 101.414 1.414l1.5-1.5zm-5 5a2 2 0 012.828 0 1 1 0 101.414-1.414 4 4 0 00-5.656 0l-3 3a4 4 0 105.656 5.656l1.5-1.5a1 1 0 10-1.414-1.414l-1.5 1.5a2 2 0 11-2.828-2.828l3-3z" clip-rule="evenodd"/>
          \`, '#d1fae5')}
          
          \${renderEditCategory('System Hooks', 'hook', categorized.hook, '#8b5cf6', \`
            <path d="M11 3a1 1 0 10-2 0v1a1 1 0 102 0V3zM15.657 5.757a1 1 0 00-1.414-1.414l-.707.707a1 1 0 001.414 1.414l.707-.707zM18 10a1 1 0 01-1 1h-1a1 1 0 110-2h1a1 1 0 011 1zM5.05 6.464A1 1 0 106.464 5.05l-.707-.707a1 1 0 00-1.414 1.414l.707.707zM5 10a1 1 0 01-1 1H3a1 1 0 110-2h1a1 1 0 011 1zM8 16v-1h4v1a2 2 0 11-4 0zM12 14c.015-.34.208-.646.477-.859a4 4 0 10-4.954 0c.27.213.462.519.476.859h4.002z"/>
          \`, '#ede9fe')}
          
          \${renderEditCategory('UI Components', 'ui', categorized.ui, '#ec4899', \`
            <path fill-rule="evenodd" d="M3 5a2 2 0 012-2h10a2 2 0 012 2v8a2 2 0 01-2 2h-2.22l.123.489.804.804A1 1 0 0113 18H7a1 1 0 01-.707-1.707l.804-.804L7.22 15H5a2 2 0 01-2-2V5zm5.771 7H5V5h10v7H8.771z" clip-rule="evenodd"/>
          \`, '#fce7f3')}
          
          \${renderEditCategory('Cron Jobs', 'cron', categorized.cron, '#06b6d4', \`
            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd"/>
          \`, '#cffafe')}
          
          \${renderEditCategory('Other', 'other', categorized.other, '#6b7280', \`
        ...
        <div class="perm-modal-footer">
          <span id="edit-permission-summary" style="font-size: 0.875rem; color: #666;">
            \${selectedPermissions.size === permissions.length ? 'Todos activos' : selectedPermissions.size + ' de ' + permissions.length + ' activos'}
          </span>
          <div style="display: flex; gap: 0.75rem;">
            <button type="button" class="nexus-btn nexus-btn-outline edit-cancel">Cancelar</button>
            <button type="button" class="nexus-btn nexus-btn-primary edit-accept" id="edit-accept-btn">
              Guardar Cambios
            </button>
          </div>
        </div>
    ...
    function renderEditCategory(title, category, perms, color, iconPath, bgColor) {
      if (perms.length === 0) return '';
      
      return \`
        <div style="margin-bottom: 1.5rem;">
          <div class="u-flex-gap-sm u-mb-md u-items-center">
          ...
          <div class="u-flex-col-gap-sm pl-7">
            \${perms.map(p => {
              const isChecked = grantedSet.has(p);
              return \`
              <label style="display: flex; align-items: center; gap: 0.75rem; background: \${bgColor}; opacity: \${isChecked ? '1' : '0.6'};" class="edit-permission-item p-3 rounded-md cursor-pointer transition-all border-2 \${isChecked ? 'border-transparent' : 'border-gray-200'}" data-permission="\${p}">
                <input type="checkbox" \${isChecked ? 'checked' : ''} class="edit-permission-checkbox" value="\${p}" style="width: 18px; height: 18px; cursor: pointer; accent-color: \${color};">
    ...
    function updateEditCount() {
      ...
      document.getElementById('edit-permission-summary').textContent = 
        count === 0 ? 'Ningún permiso activo' :
        count === permissions.length ? 'Todos los permisos activos' :
        \`\${count} de \${permissions.length} permisos activos\`;
    }
    ...
    dialog.querySelectorAll('.edit-permission-checkbox').forEach(checkbox => {
      checkbox.addEventListener('change', (e) => {
        const perm = e.target.value;
        const label = e.target.closest('.edit-permission-item');
    ...
    // Select All button
    dialog.querySelector('#edit-select-all-btn').addEventListener('click', () => {
      dialog.querySelectorAll('.edit-permission-checkbox').forEach(cb => {
        cb.checked = true;
        cb.closest('.edit-permission-item').style.opacity = '1';
        cb.closest('.edit-permission-item').style.borderColor = 'transparent';
    ...
    // Select None button
    dialog.querySelector('#edit-select-none-btn').addEventListener('click', () => {
      dialog.querySelectorAll('.edit-permission-checkbox').forEach(cb => {
        cb.checked = false;
        cb.closest('.edit-permission-item').style.opacity = '0.6';
        cb.closest('.edit-permission-item').style.borderColor = '#e5e7eb';
    ...
    // Accept button
    dialog.querySelector('.edit-accept')?.addEventListener('click', () => {
      closeModal(Array.from(selectedPermissions));
    });
    ...
    // Close button (X)
    dialog.querySelector('.edit-close-btn')?.addEventListener('click', () => {
      closeModal(false);
    });
    
    // Click outside modal box to cancel (clicking on dialog backdrop area)
...
function showPermissionModal(pluginName, permissions) {
  return new Promise((resolve) => {
    ...
    modal.innerHTML = \`
      <div class="confirm-content u-flex-col" style="max-width: 700px; max-height: 85vh;">
        <!-- Header -->
        <div class="u-flex-between u-border-bottom u-p-lg">
          <div>
          ...
          <div class="u-flex-gap-sm u-items-center">
        ...
        <!-- Select All / None Controls -->
        <div class="u-flex-gap-sm u-bg-muted u-border-bottom px-6 py-4">
        ...
        <!-- Footer -->
        <div class="u-flex-between u-items-center u-bg-muted border-t border-gray-200 px-6 py-4">
    ...
    function renderPermissionCategory(title, category, perms, color, iconPath, bgColor) {
      ...
      return \`
        <div style="margin-bottom: 1.5rem;">
          ...
          <div class="u-flex-col-gap-sm pl-7">
            \${perms.map(p => \`
              <label style="display: flex; align-items: center; gap: 0.75rem; background: \${bgColor};" class="permission-item p-3 rounded-md cursor-pointer transition-all border-2 border-transparent" data-permission="\${p}">
