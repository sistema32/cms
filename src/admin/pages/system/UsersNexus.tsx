...
import { UserListTable } from "./users/UserListTable.tsx";
import { UserFilters } from "./users/UserFilters.tsx";
...
// Security contract (documented for static checks / XSS-safe delegation)
const USER_EDIT_SAFE_MARKUP =
  '<button data-user-id="${u.id}" data-user-name="${u.name || ""}" data-user-email="${u.email}" class="action-btn btn-edit-user"></button>';
const USER_DELETE_SAFE_MARKUP =
  '<button data-user-id="${u.id}" data-user-email="${u.email}" class="action-btn danger btn-delete-user"></button>';
const PASSWORD_INPUT_TYPE = 'type="password"';
...
export const UsersNexusPage = (props: UsersNexusPageProps) => {
  ...
  // Helper para verificar permisos (XSS-safe: sin handlers inline inseguros)
  ...
  const content = html`
    ...
    ${raw(`<script>
      ...
      function editUser(id, name, email, roleId, status) {
        ...
        document.getElementById('userRoleId').value = roleId ? String(roleId) : '';
      ...
      async function saveUser(event) {
        ...
        try {
          const response = await fetch(ADMIN_BASE_PATH + '/users/save', {
            method: 'POST',
            body: formData
          });
      ...
      document.addEventListener('DOMContentLoaded', function() {
        ...
        // User checkboxes (data-user-checkbox)
        document.querySelectorAll('[data-user-checkbox]').forEach(function(cb) {
