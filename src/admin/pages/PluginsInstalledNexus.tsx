...
export const PluginsInstalledNexusPage = (props: PluginsInstalledNexusPageProps) => {
  const {
    user,
    plugins,
    stats,
  ...
  const content = html`
              ...
              <div class="plugin-actions">
                ...
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <circle cx="12" cy="12" r="3"/><path d="M12 1v6m0 6v6m-6-6h6m6 0h-6"/>
    ...
    ${raw(`<script>
      ...
      // XSS safe - Activation handler
      async function handleActivate(pluginName) {
        if (!confirm('¿Activar el plugin "' + pluginName + '"?')) return;

        try {
          const response = await fetch(ADMIN_BASE_PATH + '/api/plugins/' + pluginName + '/activate', {
            method: 'POST'
          });
      ...
      // XSS safe - Deactivation handler
      async function handleDeactivate(pluginName) {
        if (!confirm('¿Desactivar el plugin "' + pluginName + '"?')) return;

        try {
          const response = await fetch(ADMIN_BASE_PATH + '/api/plugins/' + pluginName + '/deactivate', {
            method: 'POST'
          });
      ...
      // XSS safe - Uninstall handler
      async function handleUninstall(pluginName) {
        if (!confirm('¿Desinstalar el plugin "' + pluginName + '"? Esta acción no se puede deshacer.')) return;

        try {
          const response = await fetch(ADMIN_BASE_PATH + '/api/plugins/' + pluginName, {
            method: 'DELETE'
          });
      ...
      async function openSettings(pluginName) {
        try {
          const response = await fetch(ADMIN_BASE_PATH + '/api/plugins/' + pluginName + '/settings');
