 ...
 */
adminRouter.get("/forms/new", async (c) => {
  try {
    const user = c.get("user");
 ...
 */
adminRouter.get("/forms/:id/edit", async (c) => {
  try {
    const user = c.get("user");
    const id = parseInt(c.req.param("id"));
 ...
 */
adminRouter.get("/forms/:id/submissions", async (c) => {
  try {
    const user = c.get("user");
    const id = parseInt(c.req.param("id"));
    const page = parseInt(c.req.query("page") || "1");
 ...
 */
adminRouter.post("/forms/new", async (c) => {
  try {
    const user = c.get("user");
    const body = await c.req.parseBody();
 ...
 */
adminRouter.post("/forms/edit/:id", async (c) => {
  try {
    const id = parseInt(c.req.param("id"));
    const body = await c.req.parseBody();
 ...
 */
adminRouter.post("/forms/:id/fields", async (c) => {
  try {
    ...
    const fieldData = {
      ...
      options: body.options ? (body.options as string).split('\n').filter(o => o.trim()) : undefined,
    };

    await formService.addField(formId, fieldData);

    return c.redirect(`${env.ADMIN_PATH}/forms/${formId}/edit`);
  } catch (error: any) {
    console.error("Error adding field:", error);
    return c.text("Error al agregar el campo", 500);
  }
});
