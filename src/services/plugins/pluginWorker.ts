...
export async function startWorker(
  ...
  worker.onmessage = async (e: MessageEvent<WorkerToMainMessage>) => {
    ...
    switch (msg.type) {
      ...
      case "registerRoute":
        registerRoute(
          { plugin: name, capabilities: capabilities || { dbRead: false, dbWrite: false, fsRead: false, fsWrite: false, httpAllowlist: [] } },
          {
      ...
      case "registerHook":
        ...
        registerHook({ plugin: name, capabilities: capabilities || { dbRead: false, dbWrite: false, fsRead: false, fsWrite: false, httpAllowlist: [] } }, {
      ...
      case "registerCron":
        ...
        registerCron({ plugin: name, capabilities: capabilities || { dbRead: false, dbWrite: false, fsRead: false, fsWrite: false, httpAllowlist: [] } }, {
      ...
      case "registerUiSlot":
        ...
        registerUiSlot({ plugin: name, capabilities: capabilities || { dbRead: false, dbWrite: false, fsRead: false, fsWrite: false, httpAllowlist: [] } }, msg.payload.slot, msg.payload.label, msg.payload.url);
        break;
      case "registerAsset":
        const { registerAsset } = await import("./pluginRuntime.ts");
        registerAsset({ plugin: name, capabilities: capabilities || { dbRead: false, dbWrite: false, fsRead: false, fsWrite: false, httpAllowlist: [] } }, msg.payload.type, msg.payload.url);
        break;
      case "registerWidget":
        const { registerWidget } = await import("./pluginRuntime.ts");
        registerWidget({ plugin: name, capabilities: capabilities || { dbRead: false, dbWrite: false, fsRead: false, fsWrite: false, httpAllowlist: [] } }, msg.payload.widget, msg.payload.label, msg.payload.renderUrl);
        break;
      case "routeResult":
      ...
      case "dbRequest":
        try {
          ...
          const result = await handleDbRequest(name, msg.req, capabilities || { dbRead: false, dbWrite: false, fsRead: false, fsWrite: false, httpAllowlist: [] });
      ...
      case "fetch":
        try {
          ...
          const cap = capabilities || { dbRead: false, dbWrite: false, fsRead: false, fsWrite: false, httpAllowlist: [] };
          const response = await httpFetch(msg.url, msg.init || {}, {
            allowlist: cap.httpAllowlist,
            timeoutMs: 10000,
            methods: cap.httpMethods,
          });
...
export function stopWorker(name: string) {
  ...
  if (w) {
    try {
      w.worker.postMessage({ type: "lifecycle", phase: "deactivate" });
    } catch (_e) {
      // ignore
    }
    try {
...
export function workerHasPermission(name: string, permission: string): boolean {
  const worker = workers.get(name);
  if (!worker) return false;
  return worker.permissions.has(permission);
}

// Legacy alias kept for backwards compatibility with LexCMS tests and scripts.
export const hasPermission = workerHasPermission;

export function assertPermission(name: string, permission: string) {
  if (!workerHasPermission(name, permission)) {
