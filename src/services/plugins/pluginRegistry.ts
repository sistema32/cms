...
function mapPlugin(row: typeof plugins.$inferSelect): PluginRecord {
  ...
  manifestSignature: (row as any).manifestSignature ?? null,
...
export async function registerDiscoveredPlugin(manifest: any) {
  ...
  const manifestHash = manifest.checksum ?? manifest.manifestHash ?? null;
  const manifestSignature = manifest.signature ?? manifest.manifestSignature ?? null;
  ...
  if (existing) {
    ...
    const hashChanged = manifestHash && manifestHash !== existing.manifestHash;
    const signatureChanged = manifestSignature && manifestSignature !== (existing as any).manifestSignature;

    if (!permsChanged && !versionChanged && !descChanged && !hashChanged && !signatureChanged) {
      return;
    }
    ...
    await db.update(plugins).set({
      ...
      manifestHash: manifestHash ?? existing.manifestHash,
      manifestSignature: manifestSignature ?? (existing as any).manifestSignature ?? null,
      updatedAt: new Date(),
