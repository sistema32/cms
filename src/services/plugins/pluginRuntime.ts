...
export async function openBreaker(plugin: string, reason = "breaker_open") {
  ...
  try {
    ...
    const m = recordHookMetric as any;
    m && m(plugin, 0, false);
  } catch (_e) {
...
export async function runHookSafely(hookName: string, ...args: any[]) {
  ...
  for (const h of handlers) {
    if (isBreakerOpen(h.plugin)) {
      try {
        const { recordHookMetric } = await import("./pluginMetrics.ts");
        recordHookMetric(h.plugin, 0, false);
      } catch (_e) {
        // ignore metrics errors
      }
      continue;
...
export function registerRoute(ctx: SandboxContext, route: RegisteredRoute) {
  ...
  // Also expose a plugin-agnostic key for compatibility with tests/legacy lookups
  const genericKey = `${route.method.toUpperCase()}:${route.path}`;
  runtimeRoutes.set(genericKey, route);
}

export function registerHook(ctx: SandboxContext, hook: RegisteredHook) {
  assertPermission(ctx.plugin, hook.permission);
  // Allow legacy hook:* pattern in tests while still supporting cms_ prefixed hooks
  if (!hook.name.startsWith("cms_") && !hook.name.startsWith("hook:")) {
