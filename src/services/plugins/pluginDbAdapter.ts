 ...
 */
function requireCapability(cap: SandboxCapabilities | undefined, op: "read" | "write") {
    if (!cap) return false;
    return op === "write" ? !!cap.dbWrite : !!cap.dbRead;
}

export async function handleDbRequest(pluginName: string, req: DbRequest, cap?: SandboxCapabilities) {
    const isWrite = req.operation === "insert" || req.operation === "update" || req.operation === "delete";
    if (isWrite && !requireCapability(cap, "write")) {
        throw new Error("DB sandbox: plugin does not have db:write capability");
    }
    if (!isWrite && !requireCapability(cap, "read")) {
        throw new Error("DB sandbox: plugin does not have db:read capability");
    }
