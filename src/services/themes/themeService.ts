...
export async function updateThemeCustomSettings(
  ...
  return await settingsService.updateThemeCustomSettings(themeName, customSettings);
}
...
/**
 * Activa el modo seguro (Safe Mode)
 * Revierte al theme por defecto y deshabilita personalizaciones conflictivas
 */
export async function activateSafeMode(reason?: string): Promise<boolean> {
  console.log(`⚠️ Activating Safe Mode: ${reason || "Unknown reason"}`);

  try {
    // 1. Force revert to default theme
    await settingsService.updateSetting("active_theme", "default");

    // 2. Invalidate all caches
    themeCacheService.invalidateAll();

    // 3. Log the incident
    await settingsService.updateSetting("last_safe_mode_activation", JSON.stringify({
    ...
    console.error("❌ CRITICAL: Failed to activate Safe Mode:", error);
    return false;
  }
}
...
export function getThemeCacheStats() {
  const stats = themeCacheService.getStats();
  return {
    ...stats,
    hits: stats.templates.hits + stats.configs.hits,
    misses: stats.templates.misses + stats.configs.misses,
  };
}
...
export { themeI18nService } from "./themeI18nService.ts";
...
/**
 * Exponer estadísticas del caché de themes (compatibilidad con API anterior)
 */
